...
for i in range(len(channel_list)):
    for k in range(3):
        ...
        for j in range(4):
            ...
            tmp_task_emg = np.concatenate(all_data_emg[i][k][j]).reshape(-1, 4, 5000)

            # 计算CMC
            for tmp_ind in range(len(tmp_task_eeg)):
                each_EEG_data = tmp_task_eeg[tmp_ind, :, :]
                each_EMG_data = tmp_task_emg[tmp_ind, :, :]
                for channel_ind in range(len(channels)):
                    EEG_channel_data = each_EEG_data[channel_ind, :]
                    # 计算CMC相关性
                    fs = 1000
                    for emg_channel in range(4):
                        EMG_channel_data = each_EMG_data[emg_channel, :]

                        x0 = EEG_channel_data
                        y = EMG_channel_data
                        frequencies, csd = signal.csd(x0, y, fs=fs, nperseg=1000)
                        # 计算相干性
                        coherence = np.abs(csd) ** 2 / (
                                signal.csd(x0, x0, fs=fs, nperseg=1000)[1] *
                                signal.csd(y, y, fs=fs, nperseg=1000)[1])

                        each_CMC.append(coherence)

            all_CMC.append(each_CMC)

        with open('../CMC/data'+'\\'+ name_list[i]+'_'+str(k) +'CMC.pkl', 'wb') as f:
            pickle.dump({'CMC': all_CMC,
                'frequencies': frequencies,
                'EEG_data': data_x,
                'label_data': all_data_y[i],
                'EMG_data': all_data_emg[i],
